...
export const TermsContent = () => (
  <>
    ...
    <Section number="02" title="정의">
      <SubList
        items={[
          ...
          "회사 제공 키: 회사가 보유한 OpenAI API 키로, 회원이 별도의 키를 등록하지 않아도 챗봇이 응답할 수 있도록 회사가 사용료를 부담하여 제공하는 것",
    ...
    <Section number="06" title="API 키 및 외부 LLM 이용">
      <SubList
        items={[
          "회사는 회원이 별도의 API 키를 등록하지 않아도 챗봇을 이용할 수 있도록 회사 제공 키(OpenAI)를 무상으로 제공합니다. 이 경우 사용 모델은 회사가 지정한 것으로 고정되며, 플랜별 월 대화 건수 한도가 적용됩니다.",
          "회사 제공 키로 처리되는 대화는 회사의 계정을 통해 외부 LLM 제공자에게 전달됩니다. 해당 제공자에 대한 사용료는 회사가 부담합니다.",
          "회원은 OpenAI, Anthropic, Google 등 외부 LLM 제공자의 API 키를 직접 발급받아 등록할 수 있으며(BYOK), 이 경우 모델 선택·파일 학습·웹 검색이 제공되고 월 대화 건수 한도가 적용되지 않습니다. Anthropic 및 Google 모델은 회원 키 등록 시에만 이용할 수 있습니다.",
          "회원이 등록한 키로 발생하는 외부 LLM 제공자에 대한 사용량·과금·이용약관은 해당 제공자와 회원 간의 직접적인 계약 관계이며, 회사는 이에 대한 책임을 지지 않습니다. 회원은 자신의 키로 발생하는 모든 호출과 비용에 대한 책임을 집니다.",
          "회사는 회원이 등록한 API 키를 암호화하여 저장하며, 회원의 챗봇이 응답을 생성할 때만 복호화하여 사용합니다.",
          "회사는 제공 키의 남용을 방지하기 위하여 사용 모델을 지정하고 월 대화 건수를 제한할 수 있으며, 제공 범위가 변경되는 경우 제7조 제4호에 따라 사전에 안내합니다.",
        ]}
      />
    </Section>
    ...
    <Section number="07" title="서비스 요금 및 결제">
      <SubList
        items={[
          ...
          "회사 제공 키를 이용하는 경우 LLM 사용료는 회사가 부담하며 별도로 청구되지 않습니다. 다만 플랜별 월 대화 건수 한도가 적용됩니다.",
          "회원이 자신의 API 키를 등록하여 이용하는 경우, LLM 제공자(OpenAI / Anthropic / Google)에 직접 지불하는 API 사용료는 서비스 이용료에 포함되지 않습니다.",
        ]}
      />
    </Section>
...
export const PrivacyContent = () => (
  <>
    ...
    <Section number="04" title="개인정보의 제3자 제공">
      ...
      <SubList
        items={[
          "이용자가 사전에 동의한 경우",
          "법령에 의해 제공이 요구되는 경우",
          "챗봇 응답을 생성하기 위해 외부 LLM 제공자(OpenAI, Anthropic, Google)에 메시지가 전달됩니다. 이용자가 자신의 API 키를 등록한 경우에는 이용자의 키와 계정으로 호출되고, 키를 등록하지 않아 회사 제공 키를 사용하는 경우에는 회사의 OpenAI 계정을 통해 호출됩니다. 어느 경우든 각 제공자의 개인정보처리방침이 적용됩니다. 자세한 내용은 아래 '개인정보의 국외 이전'을 참고해 주세요.",
        ]}
      />
    </Section>
    ...
    <Section number="06" title="개인정보의 국외 이전">
      ...
      <SubList
        items={[
          "OpenAI, L.L.C. (미국) — 이전 항목: 챗봇에 입력된 대화 메시지, 시스템 프롬프트, 학습 데이터, 다국어 기능 사용 시 FAQ·인사말 텍스트 / 이용 목적: 챗봇 응답 생성 및 FAQ·인사말 번역 / 보유 기간: 해당 제공자의 정책에 따릅니다. 호출 주체는 이용자가 자신의 API 키를 등록한 경우 이용자 본인의 키와 계정이며, 키를 등록하지 않아 회사 제공 키를 사용하는 경우 회사의 계정입니다.",
          "Anthropic PBC (미국) — 이전 항목·목적·보유 기간은 위와 같으며, 이용자 본인의 API 키와 계정으로만 호출됩니다(회사는 해당 제공자의 키를 제공하지 않습니다).",
